...
This cell type displays checkbox input. `checked` property contais single source of truth. This template has disabled edit mode.
...
| `checked`         | `boolean`         | Currect state of `checked` attribute in `input` element | 
| `checkedText?`    | `string`          | Text of "checked text input", used while getting compatible cell, defaultly: `1` | 
| `uncheckedText?`  | `string`          | Text of "unchecked text input",  used while getting compatible cell, defaultly: `` (empty string) | 

## `CheckboxCellTemplate` class implementation

```tsx
class CheckboxCellTemplate implements CellTemplate<CheckboxCell> {
