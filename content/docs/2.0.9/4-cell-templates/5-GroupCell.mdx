...
This cell template renders text with the preceding chevron. `GroupCell` contains data about single node in the tree structure of the data.
...
| `hasChildrens?`    | `boolean`        | Determining having children by a node element | 
| `parentId?`        | [`Id`](link)     | [`Id`](link) of parent node, if undefined node becomes root | 
