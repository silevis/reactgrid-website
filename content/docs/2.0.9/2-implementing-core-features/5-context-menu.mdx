...
ReactGrid contains built-in custom context menu with is disabled by default. You also can create your custom actions.
...
`menuOptions` argument of `simpleHandleContextMenu` contains default list of predefined options - copy, cut, paste.
...
Select column or row by click on first left or top cell, then right-click and remove selected row or column.
