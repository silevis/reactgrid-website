...
1. Firstly lets update imports by ading `Id`, `DropPosition`, `Column`, `Row`, `Cell`

```tsx
import { ReactGrid, Id, DropPosition, Column, Row, Cell } from "@silevis/reactgrid";
```

2. Prepare out state to allow row and column reorder:

Mark column as reorderable by adding `reorderable` attribute and set it to `true`

```tsx
columns: [
	// column now are able to reorder
