...
1. A user changes the value of a cell. Among others, it could mean that the user has:
   - typed a new value into the a cell by cell editor,
   - used a fill handle ,
...
1. Committing changes after cell editing.
...
Given that information, we find the row and column affected by each change,
