...
const code = `const StickyPanesSample = () => {
...
ReactGrid allows you to stick chosen rows and columns at the top or left side and additionally on right and bottom 
of the ReactGrid viewport only in Pro version. Sticky rows or columns will be visible at all times, no matter the scroll position.
 
<Alert color="warning">
	<i className="tim-icons icon-alert-circle-exc pr-2"></i>
	Right and bottom sticky is only available in Pro version
</Alert>

Sticky rows and columns can be enabled by adding dedicated `ReactGrid` component props:
- `stickyTopRows`
- `stickyBottomRows`
- `stickyLeftColumns`
- `stickyRightColumns`
...
or reduce height of grids container
...
2. Use the properties mentioned in the introduction to set the number of sticky panes at each edge
...
Here's how frozen rows and columns look in action (we recommend to play with this demo on fullscreen mode):
