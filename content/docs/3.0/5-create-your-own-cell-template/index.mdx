...
For this tutorial we are going to use previous project - [handling changes](link).
...
* `render` - return content of cell. In edit mode we will display HTML `input` element. Change that taking place in the 
...
Go back to `index.tsx` file and add `customCellTemplates` prop as shown below:
