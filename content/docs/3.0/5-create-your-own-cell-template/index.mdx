...
2. When a cell is displayed it should show country a flag based on cell text
...
Create new file named `FlagCellTemplate.tsx`, add the same imports as on listing below and `flag-cell-style.css`  that
will contain some CSS styles (don't forget to import it in your project's file).
...
For all defined methods below it is necessary to correct displaying and interfacing with internal ReactGrid model.
...
* `update` - as we are not sure that incoming cell has the same interface like `FlagCell` so we marking it
...
* `render` method returns content of cell. In edit mode we will display `input` element. This change is made in the 
...
All of the cells have a class name created based on cell `type` attribute. In our case, the cell class name will be `rg-flag-cell`. 
Due to `rg-cell` is flex element we centering its content by `justify-content: center;` attribute.
