...
1. An Undo/Redo feature isn't implemented internally, but we prepared example which you can take pattern of this.
2. ReactGrid bypassing undo and redo events.
...
We have an array of changes made by the user at each step. The place in the history of changes you are in is stored.
...
We need additionally to store all changes and changes index with help us to move on the changes array.
...
Change index is moved one step backward. 
...
```
The `redoChanges` function allows you to set the restored change. Change index also have to be moved one position forward.
...
Let's use the above functions inside example `App` component. Inside it we added two hooks:
...
The first one stores current index of change for the second hook - storing an array of changes that has been affected on 
the `TextCells`.
```
We have wrapped ReactGrid with `div` elementthat listens to `onKeyDown` event. 
Changes can be also restored/redone with two buttons.

Inside both methods (`handleUndoChanges` and `handleRedoChanges`) we updates our data - poople array.
These changes comes from `cellChanges` array.
